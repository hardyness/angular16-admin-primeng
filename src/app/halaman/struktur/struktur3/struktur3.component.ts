...
export class Struktur3Component {
  ...
  async tambah(){
    ...
    if(!this.formStruktur2.valid){
      ...
      return new Promise (resolve => {
        ...
        this.subHttp = this.api.postData(param, 'struktur3/tambah', {headers}).subscribe((res: any) => {
          if (res.status == 1){
            this.loadingButton = false;
            this.messageService.add({severity: 'error', summary: res.pesan, detail: 'Akses Anda ditolak!'});
            this.auth.logout();
  ...
  async perbarui(){
    ...
    if(!this.formStruktur2.valid){
      ...
      return new Promise (async resolve => {
        const param = new FormData();
        param.append('idstruktur3',  this.idstruktur3);
        param.append('struktur1', this.idselectstruktur1);
        var headers = new HttpHeaders({
          'x-access-token': this.sesitoken,
          'x-access-unik': this.sesiunik,
        ...
        this.subHttp = this.api.postData(param, 'struktur3/perbarui', {headers}).subscribe((res: any) => {
          if (res.status == 1){
            this.loadingButton = false;
            this.messageService.add({severity: 'error', summary: res.pesan, detail: 'Akses Anda ditolak!'});
            this.auth.logout();
  ...
  async hapusData(id){
    ...
    return new Promise (async resolve => {
      ...
      this.subHttp = this.api.postData(param, 'struktur3/hapus', {headers}).subscribe((res: any) => {
