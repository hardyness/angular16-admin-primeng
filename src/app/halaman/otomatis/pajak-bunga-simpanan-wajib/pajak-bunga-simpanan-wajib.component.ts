  ...
  async tambahDebit(){
    ...
    if(!this.formTambahDebit.valid){
      ...
      return new Promise (resolve => {
        ...
        this.subHttp = this.api.postData(param, 'pajaksimpananwajib/tambahdebitpajaksimpananwajib', {headers}).subscribe((res: any) => {
          if (res.status == 1){
            this.loadingButton = false;
            this.messageService.add({severity: 'error', summary: res.pesan, detail: 'Akses Anda ditolak!'});
            this.auth.logout();
  ...
  async tambahKredit(){
    ...
    if(!this.formTambahKredit.valid){
      ...
      return new Promise (resolve => {
        ...
        this.subHttp = this.api.postData(param, 'pajaksimpananwajib/tambahkreditpajaksimpananwajib', {headers}).subscribe((res: any) => {
          if (res.status == 1){
            this.loadingButton = false;
            this.messageService.add({severity: 'error', summary: res.pesan, detail: 'Akses Anda ditolak!'});
            this.auth.logout();
