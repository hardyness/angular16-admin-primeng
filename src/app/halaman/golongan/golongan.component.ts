...
export class GolonganComponent {
  ...
  async listData(){
    ...
    return new Promise (resolve => {
      ...
      this.subHttp = this.api.postData(param, 'golongan/list', {headers}).subscribe((res: any) => {
  ...
  async cekTambah(){
    ...
    return new Promise (() => {
      ...
      this.subHttp = this.api.postData(cekmenu, 'golongan/cek', {headers}).subscribe((res: any) => {
        if (res.status == 1){
          this.messageService.add({severity: 'error', summary: res.pesan, detail: 'Akses Anda ditolak!'});
          this.auth.logout();
        } else if (res.status == 99){
  ...
  async tambah(){
    ...
    this.unvalid = true;
    if(!this.formGolongan.valid){
      this.loadingButton = false;
      return false
    } else {
      var namaGolongan: any = this.formGolongan.value.namaGolongan.replace(/\b\w/g, (char: string) => char.toUpperCase());
      if (this.api.kataKasar(namaGolongan)){
        this.loadingButton = false;
      ...
      return new Promise (resolve => {
        ...
        paramTambah.append('golongan', namaGolongan);
        var headers = new HttpHeaders({
          'x-access-token': this.sesitoken,
          'x-access-unik': this.sesiunik,
          'akses': 'C9AC27E0492481C5E07CA7DF996811B1',
        ...
        this.subHttp = this.api.postData(paramTambah, 'golongan/tambah', {headers}).subscribe((res: any) => {
          if (res.status == 1){
            this.loadingButton = false;
            this.messageService.add({severity: 'error', summary: res.pesan, detail: 'Akses Anda ditolak!'});
            this.auth.logout();
            ...
            this.loadingButton = false;
            this.popForm = false;
            this.messageService.add({severity: 'success', summary: res.pesan, detail: 'Anda berhasil menambah data golongan!'});
            if (this.total == 0){
              this.cari = '';
              localStorage.removeItem('paging_golongan')
            }
            this.page = 1;
            this.totalinput = 0;
            this.listData();
  ...
  async dataList(id){
    ...
    return new Promise (() => {
      ...
      dataPerbarui.append('idgolongan', id);
      var headers = new HttpHeaders({
        'x-access-token': this.sesitoken,
        'x-access-unik': this.sesiunik,
        'akses': 'C9AC27E0492481C5E07CA7DF996811B1',
      ...
      this.subHttp = this.api.postData(dataPerbarui, 'golongan/data', {headers}).subscribe((res: any) => {
        if (res.status == 1) {
          ...
          this.loadingForm = false;
          this.namaGolongan = res.golongan;
        }
      }, (err) => {
        this.loadingForm = false;
        if (err.status == 401){
  ...
  async perbarui(){
    ...
    if(!this.formGolongan.valid){
      ...
      return new Promise (async resolve => {
        ...
        paramPerbarui.append('idgolongan',  this.idgolongan);
        paramPerbarui.append('golongan', namaGolongan);
        var headers = new HttpHeaders({
          'x-access-token': this.sesitoken,
          'x-access-unik': this.sesiunik,
          'akses': 'C9AC27E0492481C5E07CA7DF996811B1',
        ...
        this.subHttp = this.api.postData(paramPerbarui, 'golongan/perbarui', {headers}).subscribe((res: any) => {
          if (res.status == 1){
            this.loadingButton = false;
            this.messageService.add({severity: 'error', summary: res.pesan, detail: 'Akses Anda ditolak!'});
            this.auth.logout();
            ...
            this.loadingButton = false;
            this.messageService.add({severity: 'success', summary: res.pesan, detail: 'Anda berhasil merubah data di golongan!'});
            this.popForm = false;
            this.totalinput = 0;
            this.listData();
            let index = this.isidata.findIndex(item => item.idgolongan === this.idgolongan);
            if (index !== -1) {
              this.isidata [index].golongan = namaGolongan;
            }
          }
        }, (err) => {
          this.loadingButton = false;
  ...
  async hapusData(id){
    ...
    return new Promise (async resolve => {
      ...
      paramHapus.append('idgolongan',  id);
      var headers = new HttpHeaders({
        'x-access-token': this.sesitoken,
        'x-access-unik': this.sesiunik,
        'akses': 'C9AC27E0492481C5E07CA7DF996811B1',
      ...
      this.subHttp = this.api.postData(paramHapus, 'golongan/hapus', {headers}).subscribe((res: any) => {
        ...
        if (res.status == 1){
          ...
          this.messageService.add({severity: 'success', summary: res.pesan, detail: 'Anda berhasil menghapus data di golongan!'});
          this.totalinput = 0;
          this.listData();
          var index = this.isidata.findIndex(item => item.idgolongan === id);
          if (index !== -1){
  ...
  async konfirmHapus(id, target){
    this.idgolongan = id.idgolongan;
    this.confirmationService.confirm({
      target: target,
      message: 'yakin ingin menghapus ' + id.golongan + '?',
  ...
  //form
  async formKosong(){
    this.formGolongan.get('namaGolongan').reset();
  }
  ...
  async openPop(p1, p2){
    if (p2 == 1){
      this.cekTambah();
      this.popForm = true;
      this.namaForm = 'Tambah Golongan';
    } else if (p2 == 2){
      this.dataList(p1.idgolongan)
      this.popForm = true;
      this.namaForm = 'Perbarui Data Golongan';
      this.idgolongan = p1.idgolongan;
    }
  }
  ...
  async downloadexcel(){
    var header = ['Id golongan',  'Golongan']
    this.excel.generateExcel('Data golongan', 'golongan', header, this.isidata)
  }
}
