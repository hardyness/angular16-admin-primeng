...
export class BarangComponent {
  ...
  tablecolom = [
    ...
    {namakolom: 'kategori Barang', class: 'orderdata', order: 'kb.nama_kategori_barang'},
    {namakolom: 'Nama Barang', class: 'orderdata', order: 'lb.nama_barang'},
    {namakolom: 'Kode Barang'},
    {namakolom: 'No. Barcode'},
    {namakolom: 'No. PO'},
    {namakolom: 'Pcs Perkarton'},
    {namakolom: 'Stok Karton'},
    {namakolom: 'Stok Pcs'},
  ...
  async listData(): Promise<void> {
    ...
    try {
      const data = this.api.postData(param, "listbarang/" + this.page, { headers }, queryParams);
  ...
  async tambahbulk(): Promise<boolean> {
    ...
    const param = new FormData();
    param.append('arrdata', JSON.stringify(this.selecteddatabulk));
    const headers = new HttpHeaders({
      sesiidakun: this.sesiidakun,
      sesiusername: this.sesiusername,
      Authorization: `Bearer ${this.sesitoken}`,
    ...
    try {
      ...
      if (res.status === 1) {
        ...
        this.samedatabulk = res.data.map(data => ({barang: data.barang, kategori: data.kategori, nobarcode: data.nobarcode, nopo: data.nopo, pcsperkarton: data.pcsperkarton, stokkarton: data.stokkarton, stokpcs : data.stokpcs, hargabeli: data.hargabeli, hargajual: data.hargajual, kodebarang: data.kodebarang}));
        const set = new Set(this.samedatabulk.map(item => item.barang));
        const filteredArray = this.selecteddatabulk.filter(
          (item) => !set.has(item.barang)
        );
        this.selecteddatabulk = filteredArray;
        for (let i = 0; i < this.samedatabulk.length; i++){
          this.unselecteddatabulk.push(this.samedatabulk[i]);
        }
        return false
      } else if (res.status === 99) {
        this.popBulk = false;
  ...
  async formKosong() {
    ...
    this.formBarang.get("barcode").reset();
    ...
    reader.onload = (ev) => {
      ...
      if (jsonData[this.sheetname]) {
        const dataSheet = jsonData[this.sheetname].map(item => ({
          ...
          barang: item['NAMA BARANG'],
          kodebarang: item['KODE BARANG'],
          nobarcode: item['NO. BARCODE'],
          nopo: item['NO. PO'],
          pcsperkarton: item['PCS PERKARTON'],
          stokkarton: item['STOK KARTON'],
          stokpcs: item['STOK PCS'],
          hargabeli: item['HARGA BELI'],
          hargajual: item['HARGA JUAL']
        }));

        const categories = dataSheet.map(item => item.barang);
        ...
        if (duplicates.size > 0) {
          this.messageService.add({
            ...
            detail: `Nama barang yang terduplikasi: ${Array.from(duplicates).join(', ')}.`,
          });
  ...
  async generateFile() {
    var datadummy = [{ NO: 1, 'NAMA BARANG': 'Tulis Nama Barang' }];
    const worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(datadummy, {
      header: ['NO', 'KATEGORI BARANG', 'NAMA BARANG', 'KODE BARANG', 'NO. BARCODE', 'NO. PO', 'PCS PERKARTON', 'STOK KARTON', 'STOK PCS', 'HARGA BELI', 'HARGA JUAL'],
    });
